...
/**
 * Script para migrar dados do Laravel para o Supabase
 * Execute: php migrate-to-supabase.php
 */
...
foreach ($users as $user) {
    ...
    if ($response['status'] === 201) {
        ...
        echo "  ❌ Erro ao migrar usuário '{$user->name}': " . json_encode($response['data']) . "\n";
    }
}
...
foreach ($fileImports as $fileImport) {
    ...
    if ($response['status'] === 201) {
        ...
        echo "  ❌ Erro ao migrar importação '{$fileImport->original_filename}': " . json_encode($response['data']) . "\n";
    }
}
...
foreach ($contratos as $contrato) {
    ...
    if ($response['status'] === 201) {
        ...
        if ($errorCount <= 5) { // Mostrar apenas os primeiros 5 erros
            echo "  ❌ Erro ao migrar contrato '{$contrato->numero_contrato}': " . json_encode($response['data']) . "\n";
        }
    }
}
