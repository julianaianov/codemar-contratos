...
// GET - Download do arquivo original
export async function GET(
  ...
  try {
    ...
    // Usar arquivo original se disponível, senão usar o arquivo atual
    const arquivoParaDownload = minuta.arquivoOriginal || minuta.arquivo;
    const filePath = path.join(MINUTAS_DIR, arquivoParaDownload);
    ...
    try {
      ...
      return new NextResponse(fileBuffer, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${minuta.nome}_original.docx"`,
          'Content-Length': fileBuffer.length.toString(),
        },
      });
    } catch (fileError) {
      console.error('Erro ao ler arquivo original:', fileError);
      return NextResponse.json(
    ...
    console.error('Erro no download do original:', error);
    return NextResponse.json(
